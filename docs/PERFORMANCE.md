...
- NAS 精简版生产配置使用 1024 个视觉 token，保证小物体、空间关系和 OCR 质量；超大源图先按 `NAS_AI_VISION_PREPARE_MAX_EDGE` 和 `NAS_AI_VISION_PREPARE_MAX_MB` 转成受控 JPEG，降低传输与解码成本，而不是减少模型看到的视觉 token。
- 16 GB 以上独立显存可提升到 2 个推理请求。
- 指向同一模型服务的请求使用带交互优先级的推理门控，搜索与问答会先于后续后台索引进入队列；指向独立服务的视觉、向量和语音任务使用各自并发门控，避免无谓互相阻塞。
- `NAS_AI_CAPTION_UPGRADE_WORKERS` 控制描述升级流水线的工作线程，`NAS_AI_VISION_CONCURRENCY` 独立限制真实视觉请求。集成显卡可使用两个流水线线程重叠预处理、Embedding 和提交，同时保持单路视觉推理。
...
6. 全文结果与向量结果使用 Reciprocal Rank Fusion 合并，不依赖单一召回方式；快速结果先返回，首屏最多 4 个候选交给独立小模型精准重排，避免与视觉描述争用同一模型队列。
7. 快速扫描只更新文件发现状态；深度索引默认按 200 个文件分批，可选择均衡、最新、最早或小文件优先。
8. 夜间自动索引仅在配置的时间窗口和队列空闲时运行。内存高于正常保护线时提交正常批次；处于 `NAS_AI_BACKGROUND_MEMORY_FLOOR_MB` 与正常保护线之间时，只对旧图片描述提交最多 25 张受限探测批；低于最低水位或 Swap 不足时等待。服务重启后策略仍保留，接口会返回本轮判断原因。
